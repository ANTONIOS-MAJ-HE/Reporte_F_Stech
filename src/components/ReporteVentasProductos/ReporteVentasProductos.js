...
import VentasProductoTable from '../VentasProductoTable';

function ReporteVentasProductos() {
  const [ventasProducto, setVentas] = useState([]);
...
//   const [filtroFechaHasta, setFiltroFechaHasta] = useState('');
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        let url = 'https://reporte-b-stech.onrender.com/ventas-productos/';
        let parametros = [];
  ...
  return (
    <div>
      ...
      />
      <VentasProductoTable ventasProducto={ventasProducto} />
    </div>
  );
}
